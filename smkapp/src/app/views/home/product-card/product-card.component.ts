...
export class ProductCardComponent implements OnInit {
  ...
  removeItemCart(){
    this.itemIndex = this.productActionService.removeProductCart(this.itemIndex);
  ...
  clickIncreaseItem(productItem:any):void{
    ...
    if(this.itemAdded){
      this.quantity=this.productActionService.storage.shoppingCart[this.itemIndex].qtdIncrement();
    }
...
clickDecreaseItem(productItem:any):void{
  if (this.quantity>1){
    this.quantity--;
    if(this.itemAdded){
      this.quantity=this.productActionService.storage.shoppingCart[this.itemIndex].qtdDecrement();
    }
  }
